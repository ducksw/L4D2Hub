...
async function renderRanking(title, players, key, subTitle, porcentTitle, porcent, docId) {
    ...
    ret += `
      <div class="d-flex flex-column gap-2" style="width: 30%;">
        ...
        <div class="d-flex align-items-center gap-2">
          <a href="${player.profileurl}" target="_blank"><img src="${player.avatar}" class="image-ranking rounded" style="max-width: 100%;"></a>
          <span style="white-space: normal; word-break: break-word;">${player.displayName}</span>
        </div>
      </div>
...
async function renderLeaderboard(players, key, containerId, valueColor = 'text-danger') {
  ...
  sorted.forEach((player, index) => {
    html += `
      <div class="d-flex justify-content-between p-1">
        ...
        <b class="${valueColor}">${player[key]}</b>
      </div>
    `;
  });
...
async function renderRankingNoob(title, players, key, subTitle, docId, porcent, porcentTitle) {
...
async function init() {
  ...
  renderRanking('Elo', players, 'elo', 'Elo', 'Elo / e', calculatePorcent('elo', false, players), 'res-elo');
  renderRanking('Damage', players, 'damage', 'Damage', 'Damage / d', calculatePorcent('damage', false, players), 'res-damage');
  renderRanking('Kills', players, 'kills', 'Kills', 'Kills / k', calculatePorcent('kills', false, players), 'res-kill');
  renderRanking('Wins', players, 'win', 'Wins', 'Wins / w', calculatePorcent('win', false, players), 'res-win');
  renderRanking('Loser', players, 'losser', 'Loser', 'Loser / l', calculatePorcent('losser', false, players), 'res-loser');

  renderRankingNoob('Elo', players, 'elo', 'Elo', 'res-elo-noob', calculatePorcent('elo', true, players), 'Elo / e');
  renderRankingNoob('Damage', players, 'damage', 'Damage', 'res-damage-noob', calculatePorcent('damage', true, players), 'Damage / d');
  renderRankingNoob('Kills', players, 'kills', 'Kills', 'res-kill-noob', calculatePorcent('kills', true, players), 'Kills / k');
  renderRankingNoob('Wins', players, 'win', 'Wins', 'res-win-noob', calculatePorcent('win', true, players), 'Wins / w');
  renderRankingNoob('Loser', players, 'losser', 'Loser', 'res-loser-noob', calculatePorcent('losser', true, players), 'Loser / l');
